...
# we will embed these three features independently
...
# embedding term
E_Pclass = mt.core.Variable(dim=(k, Pclass.shape[1]), init=True, trainable=True) # (k, 3)
E_Sex = mt.core.Variable(dim=(k, Sex.shape[1]), init=True, trainable=True) # (k, 2)
E_Embarked = mt.core.Variable(dim=(k, Embarked.shape[1]), init=True, trainable=True) # (k, 4)

b = mt.core.Variable(dim=(1, 1), init=True, trainable=True) # bias

# wide
wide = mt.ops.MatMul(w, x)

# deep
embedding = mt.ops.Concat(
    mt.ops.MatMul(E_Pclass, x_Pclass), # (k, 1)
